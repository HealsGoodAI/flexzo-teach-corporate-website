  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="relative flex min-h-[80vh] items-center overflow-hidden bg-foreground">
        ...
        <div className="absolute inset-0 opacity-[0.06]" style={{ backgroundImage: "radial-gradient(circle at 70% 40%, hsl(269 97% 32% / 0.4) 0%, transparent 50%), radial-gradient(circle at 20% 80%, hsl(185 100% 55% / 0.2) 0%, transparent 40%)" }} />
          ...
          <div className="grid items-center gap-16 lg:grid-cols-2">
            <motion.div initial="hidden" animate="visible" variants={fadeUp}>
              ...
              <div className="mt-10 flex flex-wrap gap-4">
                <RegionLink href="/book-demo" className="group inline-flex items-center gap-2 rounded-md bg-[#59029F] px-8 py-4 text-sm font-semibold text-white transition-all hover:bg-[#4a0285]">{t("Book a Demo")} <ArrowRight size={16} className="transition-transform group-hover:translate-x-1" /></RegionLink>
            ...
            <motion.div initial="hidden" animate="visible" custom={2} variants={fadeUp} className="hidden lg:flex items-center justify-center">
              <div className="relative">
                <div className="h-80 w-80 rounded-full bg-[#59029F]/10 flex items-center justify-center">
                  <div className="h-56 w-56 rounded-full bg-[#59029F]/10 flex items-center justify-center">
                    <div className="h-32 w-32 rounded-full bg-[#59029F]/20 flex items-center justify-center">
                      <Brain className="h-16 w-16 text-[#59029F]" />
                    </div>
                  </div>
                </div>
              </div>
      ...
      <section className="py-28 lg:py-36">
        <div className="mx-auto max-w-7xl px-6">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={fadeUp} className="mb-16 text-center">
            <p className="text-sm font-semibold uppercase tracking-[0.25em] text-[#59029F]">{t("Built specifically for hospitals & agencies")}</p>
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            {hospitalFeatures.map((feature, i) => (
              <motion.div key={feature.title} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={i} variants={fadeUp} className="group rounded-2xl border border-border bg-background p-8 transition-all hover:border-[#59029F]/20 hover:shadow-lg">
                <div className="mb-5 flex h-12 w-12 items-center justify-center rounded-xl bg-[#59029F]/10 transition-colors group-hover:bg-[#59029F]/20"><feature.icon className="h-6 w-6 text-[#59029F]" /></div>
      ...
      <section className="relative bg-foreground py-32 lg:py-40">
        ...
        <div className="relative z-10 mx-auto max-w-4xl px-6 text-center">
          <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.8, ease: [0.25, 0.46, 0.45, 0.94] }}>
            ...
            <div className="mt-10">
              <RegionLink href="/book-demo" className="group inline-flex items-center gap-2 rounded-md bg-[#59029F] px-10 py-4 text-sm font-semibold text-white transition-all hover:bg-[#4a0285]">{t("Book a Demo")} <ArrowRight size={16} className="transition-transform group-hover:translate-x-1" /></RegionLink>
            </div>
          </motion.div>
        </div>
      </section>
