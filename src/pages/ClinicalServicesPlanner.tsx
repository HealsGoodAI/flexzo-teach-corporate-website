...
import {
  BarChart3, ShieldCheck, FlaskConical, ArrowRightLeft, LineChart, CalendarCheck, Workflow, ShieldAlert, Zap, TrendingUp, ArrowRight,
} from "lucide-react";
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="relative flex min-h-[90vh] items-center overflow-hidden bg-foreground">
        <div className="absolute inset-0 opacity-[0.03]" style={{
          backgroundImage: "radial-gradient(circle at 60% 40%, hsl(269 97% 32% / 0.4) 0%, transparent 60%)"
        }} />
        <div className="relative z-10 mx-auto w-full max-w-7xl px-6 py-32">
          <div className="grid items-center gap-16 lg:grid-cols-2">
            <motion.div initial="hidden" animate="visible" variants={fadeUp}>
              <p className="mb-6 text-sm font-semibold uppercase tracking-[0.25em] text-[#DC44CB]">Clinical Services Planner</p>
              ...
              <div className="mt-10 flex flex-wrap gap-4">
                <RegionLink href="/book-demo" className="group inline-flex items-center gap-2 rounded-md bg-[#59029F] px-8 py-4 text-sm font-semibold text-white transition-all hover:bg-[#4a0285]">
                  Request a Workshop <ArrowRight size={16} className="transition-transform group-hover:translate-x-1" />
                </RegionLink>
        ...
        <div className="mx-auto max-w-7xl px-6">
          <div className="grid gap-16 lg:grid-cols-[1fr_1.5fr]">
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={fadeUp}>
              ...
              <h2 className="mt-4 text-4xl font-bold leading-tight tracking-tight text-foreground md:text-5xl">From strategic plan<br />to operational rota</h2>
            </motion.div>
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} custom={1} variants={fadeUp} className="space-y-6 text-lg leading-relaxed text-muted-foreground">
              <p>Flexzo Clinical Services Planner helps healthcare organisations design, model and operationalise service plans so staffing matches how care is delivered. It incorporates patient pathways, case mix, throughput and skill mix requirements through clinically aware models.</p>
              <p>By translating strategic plans into operational rotas, the Planner reduces cancelled clinics, improves patient flow and ensures the right staff with the right competencies are scheduled where they are needed most.</p>
              <p>Scenario modelling supports "what if" analysis that quantifies trade-offs — the cost and continuity implications of consolidating sessions, changing clinic templates or protecting trainee opportunities.</p>
            </motion.div>
          </div>
        </div>
      </section>
          ...
          <div className="grid gap-px overflow-hidden rounded-2xl border border-primary-foreground/10 bg-primary-foreground/5 md:grid-cols-2 lg:grid-cols-3">
            {features.map((feature, i) => (
              <motion.div key={feature.title} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={i} variants={fadeUp} className="group border border-primary-foreground/5 bg-foreground p-10 transition-colors hover:bg-primary-foreground/5">
                ...
                <h3 className="mb-3 text-xl font-semibold text-primary-foreground">{feature.title}</h3>
                <p className="text-sm leading-relaxed text-primary-foreground/50">{feature.description}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="py-28 lg:py-36">
        <div className="mx-auto max-w-7xl px-6">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={fadeUp} className="mb-20 text-center">
            ...
            <h2 className="mt-4 text-4xl font-bold tracking-tight text-foreground md:text-5xl">From design to delivery</h2>
          </motion.div>
          <div className="space-y-24">
            {[
              { step: "01", title: "Model Demand", text: "Demand is modelled at the service, clinic and session level using historical activity, referral patterns and pathway timings. Clinically informed rules capture supervision ratios, procedural skill requirements and training protections.", image: cspStep1 },
              { step: "02", title: "Simulate & Compare", text: "Scenario modelling supports 'what if' analysis that quantifies trade-offs — the cost and continuity implications of consolidating sessions, changing clinic templates or protecting trainee opportunities across services.", image: cspStep2 },
              { step: "03", title: "Plan to Roster", text: "Validated service plans hand over directly into rostering and Flexzo AI forecasting, converting planning outputs into executable rotas without rework. Operational rosters reflect the exact assumptions used in planning.", image: cspStep3 },
              { step: "04", title: "Measure & Refine", text: "Analytics highlight bottlenecks, utilisation and clinic productivity. Reports track cancelled clinics, throughput, cost-per-session and waiting time reduction. The Planner learns from realised activity and refines forecasts continuously.", image: cspStep4 },
            ].map((item, i) => (
      ...
      <section className="border-t border-border bg-muted/30 py-28 lg:py-36">
        <div className="mx-auto max-w-7xl px-6">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={fadeUp} className="mb-16">
            <p className="text-sm font-semibold uppercase tracking-[0.25em] text-[#59029F]">Why Clinical Services Planner</p>
        ...
        <div className="mx-auto max-w-4xl px-6 text-center">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}>
            <h2 className="text-4xl font-bold tracking-tight text-foreground md:text-5xl">
              Request a <span className="text-[#59029F]">clinical planning workshop</span>
            </h2>
            <p className="mx-auto mt-6 max-w-xl text-lg text-muted-foreground">
              Discover how the Planner can reduce cancelled clinics, improve patient flow and align staffing with service objectives.
            </p>
