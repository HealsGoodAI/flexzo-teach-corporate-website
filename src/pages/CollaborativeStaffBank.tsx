...
import {
  ShieldCheck, Scale, Link2, BarChart3, Megaphone, Shield, TrendingDown, Zap, Gavel, FileCheck, ArrowRight, Network, Handshake, Building2, Settings,
} from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <div className="relative z-10 mx-auto w-full max-w-7xl px-6 py-32">
          <div className="grid items-center gap-16 lg:grid-cols-2">
            <motion.div initial="hidden" animate="visible" variants={fadeUp}>
              <p className="mb-6 text-sm font-semibold uppercase tracking-[0.25em] text-[#DC44CB]">{t("Collaborative Staff Bank")}</p>
          ...
          <div className="grid gap-16 lg:grid-cols-[1fr_1.5fr]">
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={fadeUp}>
              ...
              <h2 className="mt-4 text-4xl font-bold leading-tight tracking-tight text-foreground md:text-5xl">
                {t("A single operational")}<br />{t("fabric for cross-org staffing")}
              </h2>
            </motion.div>
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} custom={1} variants={fadeUp} className="space-y-6 text-lg leading-relaxed text-muted-foreground">
              <p>{t("Designed specifically for integrated care partnerships, ICSs and multi-trust networks, Flexzo Collaborative Staff Bank provides a single operational fabric for cross-organisation staffing while preserving each organisation's credentialing, pay rules and local governance.")}</p>
              <p>{t("The solution removes administrative friction with automated credential portability, trust-to-trust approvals, and audited handovers that ensure clinicians meet site-specific competencies before deployment. Flexible sharing models support temporary cover, surge capacity and planned rotations.")}</p>
              <p>{t("For clinicians, the experience is mobile-first and simple: profiles carry verified credentials and preferences, shift offers surface appropriate cross-site roles, and pay or travel variations are transparent before acceptance.")}</p>
            </motion.div>
          </div>
        </div>
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} custom={2} variants={fadeUp} className="mt-20">
          <img src={collaborativeBankMockup} alt="Collaborative Staff Bank dashboard mockup" className="w-full block" />
        </motion.div>
      </section>
      ...
      <section className="py-28 lg:py-36">
        <div className="mx-auto max-w-7xl px-6">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={fadeUp} className="mb-20 text-center">
            ...
            <h2 className="mt-4 text-4xl font-bold tracking-tight text-foreground md:text-5xl">{t("From local pools to regional fabric")}</h2>
          </motion.div>
          <div className="space-y-24">
            {[
              { step: "01", title: "Govern & Configure", text: "Implementation starts with governance workshops, mapped rulesets and a pilot to validate eligibility, approvals and payroll flows. Trust agreements, pay harmonies and escalation policies are encoded into the platform.", image: collabStep1Img },
              { step: "02", title: "Connect & Integrate", text: "Integration with local rostering, payroll and HR systems creates a near-real-time supply picture, letting planners locate available clinicians, evaluate fit and initiate transfers with minimal manual work.", image: collabStep2Img },
              { step: "03", title: "Share & Mobilise", text: "Targeted mobilisation campaigns and incentive mechanics activate clinicians across partners when demand spikes, improving responsiveness without undermining local staff pools.", image: collabStep3Img },
              { step: "04", title: "Measure & Refine", text: "Operational dashboards show utilisation heatmaps, cross-trust fill rates, credential gaps and audit trails. Continuous measurement refines priority logic and clinician incentives.", image: collabStep4Img },
            ].map((item, i) => (
      ...
      <section className="border-t border-border bg-muted/30 py-28 lg:py-36">
        <div className="mx-auto max-w-7xl px-6">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={fadeUp} className="mb-16">
            <p className="text-sm font-semibold uppercase tracking-[0.25em] text-[#59029F]">{t("Why Collaborative Staff Bank")}</p>
        ...
        <div className="mx-auto max-w-4xl px-6 text-center">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}>
            <h2 className="text-4xl font-bold tracking-tight text-foreground md:text-5xl">
              {t("Ready to build a")} <span className="text-[#59029F]">{t("shared bank")}</span>?
            </h2>
