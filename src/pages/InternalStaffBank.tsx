...
import {
  Link2, ShieldCheck, Brain, Smartphone, SlidersHorizontal, TrendingDown, Zap, ClipboardCheck, Users, HeartPulse, ArrowRight, BarChart3, RefreshCw, Building2, Settings,
} from "lucide-react";
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="relative flex min-h-[90vh] items-center overflow-hidden bg-foreground">
        ...
        <div className="absolute inset-0 opacity-[0.06]" style={{
          backgroundImage: "radial-gradient(circle at 70% 40%, hsl(269 97% 32% / 0.4) 0%, transparent 50%), radial-gradient(circle at 20% 80%, hsl(185 100% 55% / 0.2) 0%, transparent 40%)"
        }} />
          ...
          <div className="grid items-center gap-16 lg:grid-cols-2">
            <motion.div initial="hidden" animate="visible" variants={fadeUp}>
              ...
              <h1 className="text-5xl font-bold leading-[1.05] tracking-tight text-primary-foreground md:text-6xl lg:text-7xl">
                {t("Make your bank the")}{" "}<span className="text-[#59029F]">first place you look</span>
              </h1>
              <p className="mt-8 max-w-lg text-lg leading-relaxed text-primary-foreground/60">
                A purpose-built platform that turns casual and temporary workers into a reliable, first-choice source of clinical cover — reducing agency reliance and lowering cost-per-shift.
              </p>
              <div className="mt-10 flex flex-wrap gap-4">
                ...
                <RegionLink href="/contact" className="inline-flex items-center gap-2 rounded-md border border-primary-foreground/20 px-8 py-4 text-sm font-semibold text-primary-foreground transition-colors hover:border-primary-foreground/40">Contact Sales</RegionLink>
              </div>
            </motion.div>
        ...
        <div className="mx-auto max-w-7xl px-6">
          <div className="grid gap-16 lg:grid-cols-[1fr_1.5fr]">
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={fadeUp}>
              <p className="text-sm font-semibold uppercase tracking-[0.25em] text-[#59029F]">Overview</p>
              <h2 className="mt-4 text-4xl font-bold leading-tight tracking-tight text-foreground md:text-5xl">
                {t("Centralise, mobilise")}<br />{t("& engage your bank")}
              </h2>
            </motion.div>
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} custom={1} variants={fadeUp} className="space-y-6 text-lg leading-relaxed text-muted-foreground">
              <p>{t("Flexzo Staff Bank centralises onboarding, compliance, availability and engagement so that bank activity is embedded in everyday workforce planning. Managers see a single view of credential currency, shift preferences, recent activity and performance indicators, enabling confident approvals and faster fills.")}</p>
              <p>{t("Automated verification workflows streamline pre-employment checks and maintain an auditable compliance trail, reducing manual administration and protecting patient safety. Predictive prioritisation ensures that high-value internal candidates are surfaced before agency options are considered.")}</p>
              <p>{t("For multi-site organisations, the Staff Bank can operate collaboratively to share capacity across departments or partner trusts while preserving credential checks, pay rules and local approvals — increasing resilience during periods of peak demand.")}</p>
            </motion.div>
          </div>
        </div>
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} custom={2} variants={fadeUp} className="mt-20">
          <img src={staffBankMockup} alt="Staff Bank dashboard mockup" className="w-full block" />
        </motion.div>
      </section>
      ...
      <section className="py-28 lg:py-36">
        <div className="mx-auto max-w-7xl px-6">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={fadeUp} className="mb-20 text-center">
            <p className="text-sm font-semibold uppercase tracking-[0.25em] text-[#59029F]">How it works</p>
            <h2 className="mt-4 text-4xl font-bold tracking-tight text-foreground md:text-5xl">From pool to placement</h2>
          </motion.div>
          <div className="space-y-24">
            {[
              { step: "01", title: "Onboard & Verify", text: "Rapid role-based onboarding with automated competency checks and credential verification. Build an auditable, compliant bank from day one.", icon: ClipboardCheck, image: staffBankStepImg },
              { step: "02", title: "Prioritise & Match", text: "AI-enabled prioritisation surfaces the best-fit internal staff for each shift based on skills, availability, preferences and trust-specific rules.", icon: Brain, image: staffBankStep2Img },
              { step: "03", title: "Engage & Mobilise", text: "Targeted communications, personalised shift recommendations and incentive mechanics drive activation through the candidate mobile app.", icon: Smartphone, image: staffBankStep3Img },
              { step: "04", title: "Measure & Optimise", text: "Dashboards track bank utilisation, fill-times, agency replacement and staff satisfaction. Continuous refinement of prioritisation rules and incentive strategies.", icon: BarChart3, image: staffBankStep4Img },
            ].map((item, i) => (
              <motion.div key={item.step} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} custom={0} variants={fadeUp} className="grid items-center gap-12 lg:grid-cols-2">
      ...
      <section className="border-t border-border bg-muted/30 py-28 lg:py-36">
        <div className="mx-auto max-w-7xl px-6">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={fadeUp} className="mb-16">
            <p className="text-sm font-semibold uppercase tracking-[0.25em] text-[#59029F]">{t("Why Flexzo Staff Bank")}</p>
            <h2 className="mt-4 text-4xl font-bold tracking-tight text-foreground md:text-5xl">Benefits</h2>
          </motion.div>
        ...
        <div className="mx-auto max-w-4xl px-6 text-center">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}>
            <h2 className="text-4xl font-bold tracking-tight text-foreground md:text-5xl">
              {t("Ready to build your")} <span className="text-[#59029F]">{t("staff bank")}</span>?
            </h2>
            ...
            <div className="mt-10 flex flex-wrap justify-center gap-4">
              <RegionLink href="/book-demo" className="group inline-flex items-center gap-2 rounded-md bg-primary px-10 py-4 text-sm font-semibold text-primary-foreground transition-all hover:bg-[#59029F] hover:text-white">
                Book a Demo <ArrowRight size={16} className="transition-transform group-hover:translate-x-1" />
              </RegionLink>
              <RegionLink href="/contact" className="inline-flex items-center gap-2 rounded-md border border-border px-10 py-4 text-sm font-semibold text-foreground transition-colors hover:bg-muted">Contact Us</RegionLink>
            </div>
          </motion.div>
        </div>
      </section>
