  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="relative min-h-screen flex items-center overflow-hidden bg-foreground">
        <div className="absolute inset-0 bg-cover bg-center bg-no-repeat opacity-20" style={{ backgroundImage: `url(${privateHero})` }} />
          ...
          <div className="grid gap-16 lg:grid-cols-[1fr_1.5fr]">
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={fadeUp}>
              ...
              <h2 className="mt-4 text-4xl font-bold leading-tight tracking-tight text-foreground md:text-5xl">
                Modern workforce platform for{" "}<span className="text-[#59029F]">private healthcare</span>
              </h2>
            </motion.div>
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} custom={1} variants={fadeUp} className="space-y-6 text-lg leading-relaxed text-muted-foreground">
              <p>Private hospitals are under increasing pressure to maintain safe staffing levels, meet patient demand, and deliver efficient elective care — all while controlling costs.</p>
              <p>Flexzo.ai provides a modern workforce platform designed to help private healthcare providers access compliant clinical professionals quickly, reduce dependency on agencies, and manage staffing more effectively.</p>
            </motion.div>
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl px-6">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={fadeUp} className="max-w-5xl">
            <h2 className="text-4xl font-bold leading-[1.15] tracking-tight text-foreground md:text-6xl lg:text-7xl">
              Access a large database of{" "}<span className="text-[#59029F]">compliant professionals</span>
            </h2>
            <p className="mt-8 max-w-3xl text-xl leading-relaxed text-muted-foreground md:text-2xl">
              Gain access to a growing pool of healthcare professionals who are pre-vetted, compliance-ready, and available to work. Flexzo.ai enables private hospitals to source and book staff directly through an intelligent workforce platform.
            </p>
            <div className="mt-12"><BookDemoButton /></div>
          </motion.div>
        </div>
      </section>
      ...
      <section className="border-t border-border bg-muted/30 py-28 lg:py-36">
        <div className="mx-auto max-w-7xl px-6">
          <div className="grid gap-12 lg:grid-cols-3">
            {[
              { icon: TrendingDown, title: "Eliminate unnecessary agency mark-ups", desc: "Flexzo.ai operates on a subscription model — meaning you avoid expensive agency premiums and maintain predictable staffing costs." },
              { icon: Brain, title: "AI-powered matching and shift booking", desc: "Our platform uses intelligent matching to connect you with the most suitable clinicians for your vacancies, faster." },
              { icon: Shield, title: "Built for compliance and audit readiness", desc: "Upload documentation once and Flexzo.ai supports the process through automated compliance tracking, reminders, and verification workflows." },
            ].map((f, i) => (
      ...
      <section className="py-28 lg:py-36">
        <div className="mx-auto max-w-7xl px-6">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={fadeUp} className="mb-16 max-w-3xl">
            <p className="text-sm font-semibold uppercase tracking-[0.25em] text-[#59029F]">Built for Modern Private Healthcare</p>
            <h2 className="mt-4 text-4xl font-bold tracking-tight text-foreground md:text-5xl">Staffing solutions that are</h2>
          </motion.div>
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            {modernStaffing.map((item, i) => (
              <motion.div key={item.title} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={i} variants={fadeUp} className="group rounded-2xl border border-border bg-background p-8 transition-all hover:border-[#59029F]/20 hover:shadow-lg">
                <div className="mb-5 flex h-12 w-12 items-center justify-center rounded-xl bg-[#59029F]/10 transition-colors group-hover:bg-[#59029F]/20">
                  <item.icon className="h-6 w-6 text-[#59029F]" />
                </div>
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} custom={4} variants={fadeUp} className="mt-12"><BookDemoButton /></motion.div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl px-6">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={fadeUp} className="mb-16 max-w-3xl">
            <h2 className="text-4xl font-bold tracking-tight text-foreground md:text-5xl">
              One Platform.{" "}<span className="text-[#59029F]">Endless Staffing Solutions.</span>
            </h2>
            <p className="mt-4 text-lg text-muted-foreground">Flexzo.ai supports private hospitals with:</p>
          </motion.div>
          <div className="grid gap-6 md:grid-cols-2">
            {platformFeatures.map((f, i) => (
              <motion.div key={f.title} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={i} variants={fadeUp} className="flex items-start gap-4 rounded-2xl border border-border bg-background p-8 transition-shadow hover:shadow-md">
                <CheckCircle2 className="mt-1 h-6 w-6 shrink-0 text-[#59029F]" />
                <div>
          ...
          <div className="grid gap-16 lg:grid-cols-[1fr_1.5fr]">
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={fadeUp}>
              <h2 className="text-3xl font-bold leading-tight tracking-tight text-foreground md:text-5xl">
                Why Private Hospitals Choose{" "}<span className="text-[#59029F]">Flexzo.ai</span>
              </h2>
            </motion.div>
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} custom={1} variants={fadeUp}>
              <p className="mb-6 text-lg text-muted-foreground">Private hospitals are increasingly choosing Flexzo.ai because it provides:</p>
              <ul className="space-y-5">
      ...
      <section className="py-28 lg:py-36">
        <div className="mx-auto max-w-4xl px-6 text-center">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}>
            ...
            <h2 className="text-4xl font-bold tracking-tight text-foreground md:text-5xl">
              Book a <span className="text-[#59029F]">Demo</span>
            </h2>
            <p className="mx-auto mt-6 max-w-2xl text-lg text-muted-foreground">
              Want to see how Flexzo.ai can help your private hospital reduce agency spend and improve staffing efficiency? Contact our team today to arrange a personalised demo.
            </p>
            <p className="mx-auto mt-4 max-w-2xl text-muted-foreground">
              Discover how Flexzo.ai can support your workforce planning, improve fill rates, and streamline staffing operations.
            </p>
            <div className="mt-12 flex flex-wrap justify-center gap-4">
              <BookDemoButton />
              <RegionLink href="/contact" className="inline-flex items-center gap-2 rounded-md border border-border px-8 py-4 text-sm font-semibold text-foreground transition-colors hover:bg-muted">Contact Sales</RegionLink>
            </div>
          </motion.div>
        </div>
      </section>
